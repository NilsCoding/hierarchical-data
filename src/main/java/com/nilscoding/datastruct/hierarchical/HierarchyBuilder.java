...
public final class HierarchyBuilder {
     ...
     * @param childWriteMethod method to write child element list (mostly an empty one),
     *                         optional if childReadMethod always returns a list
     * @param objList          list with elements
     * @return root list
     */
    public static <K, T> List<T> build(Function<T, K> keyMethod,
                                       Function<T, K> parentKeyMethod,
                                       Function<T, List<T>> childReadMethod,
                                       BiConsumer<T, List<T>> childWriteMethod,
                                       List<T> objList) {
        if ((objList == null) || (objList.isEmpty() == true)) {
            return null;
        }
        IHierarchyDataAccessor<K, T> accessor = FunctionBasedHierarchicalDataAccessor.of(keyMethod,
                parentKeyMethod,
                childReadMethod,
                childWriteMethod);
